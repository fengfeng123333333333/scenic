<template>
	<view class="member-page" :style="$themeStyle">
		...
		<template v-else>
          ...
          ticketInfo.TicketName
			...
			<view class="member-form">
				...
				<uni-forms v-for="(item, index) in formData" :key="index" :ref="(el) => setFormRef(el, index)"
					:rules="rules" label-align="right" :model="item" label-width="80px">
					<view class="member-form__card">
						<!-- 姓名 -->
						<uni-forms-item label="姓名" name="name">
							<input class="member-form__input" type="text" v-model="item.name" placeholder="请输入姓名"
								placeholder-style="color: var(--color-disabled)" />
						</uni-forms-item>

						<!-- 电话 -->
						<uni-forms-item label="电话" name="phone">
							<input class="member-form__input" type="number" v-model="item.phone" placeholder="请输入电话"
								placeholder-style="color: var(--color-disabled)" maxlength="11" />
						</uni-forms-item>

						<!-- 身份证 -->
						<uni-forms-item label="身份证" name="identity">
							<input class="member-form__input" type="text" v-model="item.identity" placeholder="请输入身份证号码"
								placeholder-style="color: var(--color-disabled)" maxlength="18" />
						</uni-forms-item>
					</view>
				</uni-forms>
			</view>
		...
		<view class="bottom-bar" v-if="pageReady">
			...
			<button class="bottom-bar__btn" :class="{ 'bottom-bar__btn--loading': submitting }" :disabled="submitting"
				@click="handleSubmit">

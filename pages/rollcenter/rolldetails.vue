...
<template>
	<view class="rolldetails-page" :style="$themeStyle">
		...
		<template v-else>
			<!-- 券包列表 -->
			<view class="rolldetails-page__section-title">优惠券列表({{ couponCount }}张)</view>
			<view class="rolldetails-page__coupon-card" v-for="(item, index) in tabList" :key="index">
				<view class="rolldetails-page__coupon-left">
					<image class="rolldetails-page__coupon-img" :src="item.CouponImgUrl" mode="aspectFill" />
			...
			<template v-if="peart.Price > 0">
				...
				<view class="rolldetails-page__price-row">
					<text class="rolldetails-page__price-value">￥{{ displayPrice }}</text>
					<u-number-box v-model="quantity" :min="1" :max="100" @change="onQuantityChange"
						:disabledInput="true">
						<template #minus>
			...
			<view class="rolldetails-page__btn-area">
				<view class="rolldetails-page__btn" :class="{ 'rolldetails-page__btn--disabled': payLoading }"
					@click="handleSubmit">
	...
	async function handleSubmit() {
		...
		try {
			...
			if (res.data.Code === 300) {
				...
				showToast(res.data.Message || "支付取消或失败");
				return;
			}

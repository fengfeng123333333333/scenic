	...
	<view class="timeticket-page" :style="$themeStyle">
		...
		<template v-else>
			...
			<view class="timeticket-page__banner">
				<swiper class="timeticket-page__swiper" circular :indicator-dots="banList.length > 1"
					:autoplay="autoplay" :interval="interval" :duration="duration">
					<swiper-item v-for="(item, index) in banList" :key="index">
						<image class="timeticket-page__banner-img" :src="item.ImgUrl" mode="aspectFill" />
					</swiper-item>
				</swiper>
			</view>
			...
			<view class="timeticket-page__section" v-if="isShow">
				...
				<view class="timeticket-page__ticket-card" v-for="(item, index) in tablist" :key="index" v-else>
					...
					<view class="timeticket-page__ticket-body">
						...
						<view class="timeticket-page__ticket-row">
							<view class="timeticket-page__ticket-tags">
								<text class="timeticket-page__ticket-tag" v-for="(label, li) in item.LabelList"
									:key="li">{{ label }}</text>
							</view>
							<view class="timeticket-page__ticket-prices">
								...
								<text class="timeticket-page__ticket-original"
									v-if="item.MarketPrice">原价￥{{ item.MarketPrice }}</text>
							</view>
						</view>

						<view class="timeticket-page__ticket-actions">
							<view class="timeticket-page__ticket-notice" @click="showNotice(index)">
								<text>订票须知</text>
								<u-icon name="arrow-right" color="var(--color-text-secondary)" size="16" />
							</view>
							<view class="timeticket-page__ticket-btn" @click="goToDetail(item.TicketID)">
								立即预定
							</view>
						</view>
					</view>

  ...
  <view class="ticket-page" :style="$themeStyle">
    ...
    <view class="ticket-page__card">
      ...
      <view class="ticket-page__item" v-for="(row, idx) in infoRows" :key="idx">
        ...
        <text
          class="ticket-page__item-value"
          :class="{ 'ticket-page__item-value--highlight': row.highlight }"
          >{{ row.value }}</text
        >
      </view>
      ...
      <view class="ticket-page__actions" v-if="hasActions">
        <view
          class="ticket-page__btn"
          v-if="orderList.AllowRefund"
          @click="goRefund"
          >退票</view
        >
        <view
          class="ticket-page__btn"
          v-if="orderList.AllowTakeFace"
          @click="goSupplement"
          >补录</view
        >
        <view
          class="ticket-page__btn ticket-page__btn--primary"
          v-if="orderList.PayStatus == -2"
          @click="handlePay"
          >立即支付</view
        >
        <view
          class="ticket-page__btn"
          v-if="orderList.MiddleStatus == 4 && orderList.Status != 1"
          @click="goInvoice"
          >开发票</view
        >
        <view
          class="ticket-page__btn"
          v-if="orderList.MiddleStatus == 5"
          @click="goInvoice"
          >查看发票</view
        >
      </view>
...
const infoRows = computed(() => {
  ...
  const rows = [
    { label: "联系人:", value: o.Contacts || "" },
    { label: "手机号:", value: o.ContactPhone || "" },
    { label: "订单号:", value: o.OrderNo || "" },
    { label: "购买数:", value: o.TotalNumber || "" },
    { label: "已用数:", value: o.UseNumber || "" },
    { label: "游玩日期:", value: o.TravelTime || "", highlight: true },
    { label: "有效日期:", value: o.EffectiveDate || "", highlight: true },
    { label: "下单时间:", value: o.CreateTime || "" },
    { label: "物流单号:", value: o.ExpressNumber || "" },
  ];
  if (o.OrderSubType == 9)
    rows.push({ label: "排队号:", value: o.TakeNumber || "" });
  return rows;
});
...
function goInvoice() {
  ...
  uni.navigateTo({ url: `/pages/ticketCode/Invoice?url=${encoded}` });
}
  ...
  try {
    ...
    if (res.data?.Code === 200) {
      ...
      uni.requestPayment({
        ...
        success: () => {
          uni.reLaunch({ url: "/pages/index/index?tab=1&status=0" });
        },
        fail: () => {
          showToast("支付取消或失败");
        },

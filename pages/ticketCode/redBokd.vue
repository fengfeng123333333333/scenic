  ...
  <view class="face-page" :style="$themeStyle">
    ...
    <view class="face-card" v-if="faceList.length">
      <view class="face-card__header">
        ...
        <text class="face-card__count"
          >X {{ faceList[0].OTATickeTcodeFaceList.length }}</text
        >
      </view>
      <text class="face-card__pending"
        >待录人脸：{{ faceList[0].WaitFaceNumber * faceList.length }}</text
      >
    </view>

    <!-- 人脸录入列表 -->
    <view
      class="face-card face-card--entry"
      v-for="(group, gIdx) in faceList[0].OTATickeTcodeFaceList"
      :key="gIdx"
    >
      ...
      <view class="face-card__photos">
        <view
          class="face-card__photo"
          v-for="(face, fIdx) in group.OTATickeTcodeFaceList"
          :key="fIdx"
          @click="handleTakePhoto(face, fIdx)"
        >
          <image
            class="face-card__photo-img"
            :src="face.FaceUrl || face.imSrc || defaultFace"
            mode="aspectFill"
          />
        </view>
      </view>
    ...
    <!-- 提交按钮 -->
    <view
      class="face-page__submit"
      :class="{ 'face-page__submit--disabled': submitting }"
      @click="handleSubmit"
    >
      <u-loading-icon
        v-if="submitting"
...
async function fetchFaceInfo(orderCode) {
  ...
  try {
    ...
    const res = await uni.$myRequest({
      ...
      method: "POST",
      data: { openId: openid?.openid || "", Code: orderCode },
    });

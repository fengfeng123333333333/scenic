<template>
	<view class="order-content" :style="$themeStyle">
		...
		<u-navbar title="订单列表" :placeholder="true" :leftIconSize="0" :autoBack="false" :fixed="false" />

		<!-- 订单状态筛选 Tab -->
		<f-tabs v-model="activeTab" :tabs="statusTabs" field="name" @change="onStatusChange"
			lineColor="var(--color-primary)" :scroll="false" />
			...
			<view class="order-card" v-for="(item, index) in orderList" :key="index" @click="goDetail(item)">
				<view class="order-card__header">
					...
					<text class="order-card__status"
						:style="getStatusStyle(item.StatusDesc)">{{ item.StatusDesc }}</text>
				</view>
				<view class="order-card__body">
					<image class="order-card__thumb" :src="item.ProductImgUrl" mode="aspectFill" />
					<view class="order-card__info">
						<text class="order-card__name">{{ item.ProductName }}</text>
						<view class="order-card__meta">
							<text class="order-card__label">游玩日期：{{ item.TravelTime }}</text>
							<text class="order-card__label">数量：{{ item.TotalNumber }}</text>
						</view>
...
<script setup>
	...
	function goDetail(item) {
		// TODO: 根据实际路由调整
		uni.navigateTo({
			url: `/pages/ticketCode/ticketCode?orderID=${item.OrderID}`,
		});
	}

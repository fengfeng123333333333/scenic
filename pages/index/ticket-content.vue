<template>
	<view class="ticket-content" :style="$themeStyle">
		...
		<view class="ticket-banner">
			<swiper class="ticket-banner__swiper" circular :indicator-dots="indicatorDots" :autoplay="autoplay"
				:interval="interval" :duration="duration">
				<swiper-item v-for="(item, index) in banList" :key="index">
					<image class="ticket-banner__image" :src="item.ImgUrl" mode="aspectFill" />
				</swiper-item>
			</swiper>
		</view>
		...
		<view class="ticket-section" v-if="isShow">
			<f-tabs v-model="activeTab" :tabs="tabs" field="name" @change="changeTab" lineColor="var(--color-primary)"
				:scroll="scroll" />

			<Kong v-if="!requestLoading && tablist.length === 0" text="当前暂无可售票项" />
			<view class="ticket-card" v-for="(item, index) in tablist" :key="index" v-else>
				...
				<view class="ticket-card__body">
					...
					<view class="ticket-card__row">
						<view class="ticket-card__tags">
							<text class="ticket-card__tag" v-for="(label, li) in item.LabelList"
								:key="li">{{ label }}</text>
						</view>
						<view class="ticket-card__prices">
							...
							<text class="ticket-card__original">原价￥{{ item.MarketPrice }}</text>
						</view>
					</view>
		...
		<!-- 订票须知弹窗 -->
		<u-popup ref="popupRef" :show="show" :round="10" mode="bottom" @close="close" closeable
...
<script setup>
	...
	function tickDetails(item) {
		uni.navigateTo({
			url: `/pages/details/details?tId=${item.TicketID}`
		});
	}

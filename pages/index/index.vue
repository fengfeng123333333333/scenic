  ...
  <view class="page-index" :style="$themeStyle">
    ...
    <template v-else>
      ...
      <view class="passenger-notice" v-if="weather.PassengerFlow != ''">
        <u-notice-bar :text="String(weather.PassengerFlow)"></u-notice-bar>
      </view>

      <!-- 模块 4：信息卡（开放时间 / 地址 / 电话）1 -->
      <view class="info-card">
        ...
        <view class="row row--clickable" @click="dizhi">
          <view class="row__icon row__icon--secondary">
            <u-icon name="map" color="var(--color-primary)" size="28"></u-icon>
          </view>
          <view class="row__content">
              ...
              >
                ...
                <view class="product-card__body">
                  ...
                  <view class="product-card__price-row">
                    <text class="price">
                      <text class="price__symbol">￥</text>{{ item.SellPrice }}
                    </text>
            ...
            <view class="product-grid tickList" v-else>
              ...
              >
                ...
                <view class="product-card__body">
                  ...
                  >
                    <text class="price">
                      <text class="price__symbol">￥</text>{{ item.Price }}
                    </text>
                    <text class="discount">￥{{ item.MarketPrice }}</text>
                  </view>
      ...
      <u-popup ref="popup" :mask-click="false">
        <view class="modal scalVie">
          <scroll-view class="warp modal__scroll" scroll-y="true">
            ...
            <u-parse :content="notbcLit.Content"></u-parse>
          </scroll-view>
...
const pageReady = ref(false); // 全页内容就绪标志：API 请求期间展示骨架屏，完成后切换真实内容
...
async function payLoad() {
  try {
    ...
    // 骨架屏延时关闭：确保所有 API 及 200ms 延迟下的 changeTab 都已触发
    // 不管请求成功还是失败，都结束骨架屏展示
    setTimeout(() => {
...
/* ========== 模块 2：Navbar（设计稿：高 48–56px，左右 padding 16px） ========== */
...
:deep(.u-navbar__content__left:active) {
  transform: scale(0.96);
  transition: transform 120ms ease-in-out;
}

/* ========== 模块 3：Hero 轮播（设计稿：高屏宽*0.38–0.44，390px 下约 150–170px → 300–340rpx） ========== */
.hero {
  position: relative;
  width: 100%;
  height: 42vw;
...
/* 轮播顶部半透明信息浮卡（设计稿：卡片内边距 12–16px→24–32rpx，label 宽 72px→144rpx） */
.caption {
  position: absolute;
  top: 32rpx;
  left: 32rpx;
...
/* 轮播指示器：6–8px 小圆点（设计稿 → 12–16rpx），激活主色 */
...
/* Hero 内公告栏（玻璃样式已内置在 f-notice 组件中） */
.hero__notice {
  width: 80%;
  position: absolute;
  bottom: 32rpx;
...
/* ========== 模块 4：信息卡（设计稿：label 13px→26rpx，value 14px→28rpx，label 宽 72px→144rpx，图标容器 56px→112rpx） ========== */
.info-card {
  margin: var(--space-6, 48rpx) 32rpx 0;
  padding: 24rpx 32rpx;
...
/* ========== 模块 5：功能入口网格 4 列（设计稿：gap 12–16px→24–32rpx，图标容器 56px→112rpx） ========== */
.icon-grid {
  display: grid;
...
/* ========== 模块 6：Tab 导航（设计稿：高 48px→96rpx，文字 15px→30rpx semibold） ========== */
.tab-nav {
  margin: var(--space-6, 48rpx) 32rpx 0;
  background-color: var(--color-bg-card);
  border-radius: var(--radius-card);
...
:deep(.tab-nav .v-tabs__container) {
  align-items: center;
}

:deep(.tab-nav .v-tabs__container-item) {
  font-size: var(--font-size-h2, 30rpx) !important;
...
:deep(.tab-nav .v-tabs__container-item.active) {
  color: var(--color-bg-card) !important;
  background-color: var(--color-primary) !important;
  font-weight: 600 !important;
}

:deep(.tab-nav .v-tabs__container-line) {
  display: none !important;
}

/* ========== 模块 7：商品/票务网格 2 列（设计稿：gap 12–16px→24–32rpx，标题 14px→28rpx，价格 16px→32rpx） ========== */
.product-grid {
  display: grid;
...
/* ========== 模块 8：页脚品牌信息（设计稿：12px→24rpx，间距 24–32px→48–64rpx） ========== */
.footer-info {
  margin-top: 64rpx;
...
/* ========== 模块 10：弹窗 Modal（设计稿：radius 14px→28rpx，内边距 16px→32rpx） ========== */
.modal {
  position: relative;
  width: 85vw;
  max-width: 600rpx;

	...
	<view class="payment-page" :style="$themeStyle">
		...
		<template v-else>
			...
			<view class="payment-page__methods">
				<!-- 微信支付 -->
				<view class="payment-page__method" :class="{ 'payment-page__method--active': payType === 0 }"
					@click="payType = 0">
					...
					<view class="payment-page__method-radio"
				...
				<!-- 会员支付 -->
				<view v-if="tickinfo.isMember == true" class="payment-page__method"
					:class="{ 'payment-page__method--active': payType === 1 }" @click="payType = 1">
					<view class="payment-page__method-left">
						<u-icon name="account-fill" color="var(--color-secondary)" size="22" />
			...
			<view class="payment-page__footer">
				<button class="payment-page__btn" :class="{ 'payment-page__btn--loading': payLoading }"
					:disabled="payLoading" @tap="handlePay">
...
<script setup>
	...
	const PAY_API_MAP = {
		orderProde: "/api/Applets/VirtualAppletsPayGoodsOrder",
		cartime: "/api/Applets/AppletsPayTimeTicketOrder",
	};

	/** 默认支付 API */
	const DEFAULT_PAY_API = "/api/Applets/VirtualAppletsPayTicketOrder";
	...
	function handlePaySuccess(resData) {
		...
		if (payType.value === 0) {
			...
			} else if (data.pay_type === "040") {
				handleEmbeddedMiniProgram(data)
			} else {
				// JSAPI 微信支付（调起收银台）
				handleJsapiPay(data);
			}

  ...
  <view class="map-page" :style="$themeStyle">
    ...
    <template v-else>
      ...
      <view class="map-page__tabs">
        <scroll-view scroll-x class="tabs-scroll">
          <view
            class="tabs-item"
            :class="{ 'tabs-item--active': activeTab === index }"
          ...
          >
            <image
              ...
              "
              mode="aspectFit"
              class="tabs-item__icon"
            />
            <text
              class="tabs-item__label"
              :class="{ 'tabs-item__label--active': activeTab === index }"
              >{{ item.Name }}</text
            >
          </view>
        </scroll-view>
    ...
    <view class="map-page__popup" v-if="popupVisible">
      ...
      <view class="popup-body">
        <image
          :src="guide.DefaultsImgUrl"
          mode="aspectFill"
          class="popup-body__thumb"
        />
...
async function switchTab(item, index) {
  ...
  try {
    const res = await uni.$myRequest({
      ...
      method: "POST",
      data: { MapNavBarID: item.MapNavBarID },
    });
...
function addMarkers(guides) {
  ...
  mapContext.addMarkers({ markers, clear: true });
}
...
async function onMarkerTap(e) {
  try {
    const res = await uni.$myRequest({
      ...
      method: "POST",
      data: { VoiceGuideID: e.markerId || e.detail?.markerId },
    });
    const data = res?.data?.Data || {};
    guide.value = data;

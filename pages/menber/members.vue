	...
	<view class="members-page" :style="$themeStyle">
		...
		<template v-else>
			...
			<view class="members-page__collapse" v-else>
				<u-collapse accordion @change="onChange" @close="onClose" @open="onOpen">
					<u-collapse-item v-for="(item, index) in memberLis" :key="index" :title="item.ItemName">
						<view class="members-page__qrcode" @click="
                showQrcode = true;
                currentQrcode = item.TicketCode;
              ">
							<u-qrcode v-if="activeIndex == index && collapseStatus === 'open'" cid="qrcode"
								:val="item.TicketCode" :size="120" />
						</view>
					</u-collapse-item>
				</u-collapse>
			</view>
		...
		<!-- 二维码放大弹窗 -->
		<u-popup :show="showQrcode" mode="center" round="20" closeable @close="showQrcode = false">
			<view class="popup-qrcode">
				<u-qrcode v-if="showQrcode" cid="bigQrcode" :val="currentQrcode" :size="240" />
			</view>
		</u-popup>
	</view>
</template>

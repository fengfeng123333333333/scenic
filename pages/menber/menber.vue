	...
	<view class="menber-page" :style="$themeStyle">
		...
		<template v-else>
			...
			<view class="menber-page__banner">
				<swiper class="menber-page__swiper" circular :indicator-dots="banList.length > 1" :autoplay="autoplay"
					:interval="interval" :duration="duration">
					<swiper-item v-for="(item, index) in banList" :key="index">
						<image class="menber-page__banner-img" :src="item.ImgUrl" mode="aspectFill" />
					</swiper-item>
				</swiper>
			</view>
			...
			<view class="menber-page__section">
				<f-tabs v-model="activeTab" :tabs="tabs" field="name" :scroll="scroll" lineColor="var(--color-primary)"
					activeColor="var(--color-primary)" @change="onTabChange" />

				<Kong v-if="pageReady && tablist.length === 0" text="当前暂无可售票项" />
				<view class="menber-page__ticket-card" v-for="(item, index) in tablist" :key="index" v-else>
					...
					<view class="menber-page__ticket-body">
						...
						<view class="menber-page__ticket-row">
							<view class="menber-page__ticket-tags">
								<text class="menber-page__ticket-tag" v-for="(label, li) in item.LabelList"
									:key="li">{{ label }}</text>
							</view>
							<view class="menber-page__ticket-prices">
								...
								<text class="menber-page__ticket-original"
									v-if="item.MarketPrice">原价￥{{ item.MarketPrice }}</text>
							</view>
						</view>

						<view class="menber-page__ticket-actions">
							<view class="menber-page__ticket-notice" @click="showNotice(index)">
								<text>订票须知</text>
								<u-icon name="arrow-right" color="var(--color-text-secondary)" size="16" />
							</view>
							<view class="menber-page__ticket-btn" @click="goToRecharge(item.TicketID)">
								立即预定
							</view>
						</view>
					</view>
		...
		<!-- 订票须知弹窗 -->
		<u-popup :show="showPopup" :round="10" mode="bottom" @close="closePopup" closeable customStyle="width:100%">
